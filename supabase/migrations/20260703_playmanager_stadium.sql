-- PlayManager: stadium capacity 22k → 100k, driven by the ARENA facility level.
--
-- The arena facility (pm_facilities.sprite_key = 'arena') already had an upgrade
-- flow + a "სტადიონის მენეჯმენტი" UI, but capacity was a stub (level * 1000) and
-- the league sim hard-capped attendance at 45 000. This wires the arena level to a
-- real 5-tier capacity and makes the sim's attendance a DEMAND FRACTION of that
-- capacity (mirrors the TS preview getProjectedAttendance in economy.ts, so the
-- projected gate == the actual gate). Bigger ground ⇒ more seats sold ⇒ more gate
-- income, and ticket price becomes a real lever again. No new column/table: the
-- arena facility IS the stadium.
--
...
--   3             55 000
--   4             75 000
--   5+           100 000   (capacity plateaus; arena can still level for other perks)
--
-- TS mirror: src/lib/playmanager/economy.ts getStadiumCapacity.

create or replace function public.pm_stadium_capacity(
  p_level smallint
) returns integer
language sql
immutable
...
grant execute on function public.pm_stadium_capacity(smallint) to authenticated, service_role;

-- League sim: read the team's arena level → capacity; attendance = demand × cap.
-- Full function re-applied; only the v_arena_level read + attendance block + the
-- capacity/stadiumLevel return keys changed vs 20260630_playmanager_position_fit.
...
begin
  ...
  -- Attendance scales with stadium (arena) capacity; mirrors TS getProjectedAttendance.
