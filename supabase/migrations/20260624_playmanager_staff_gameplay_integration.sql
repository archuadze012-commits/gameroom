...
begin
  ...
  v_position := upper(coalesce(v_player.position, ''));
  ...
  v_new_ovr := least(99, v_player.ovr_current + v_ovr_gain);
...
begin
  ...
  select coalesce(sum(level * 8), 0)::integer
  into v_doctor_recovery_pct
  ...
  set
    ...
    morale = least(100, p.morale + least(8, v_days * 2))
