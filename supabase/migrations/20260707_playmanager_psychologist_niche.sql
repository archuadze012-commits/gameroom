-- PlayManager: psychologist's unique niche. Their morale work scales with PRESSURE
-- (team form): it matters most in a bad run / when the team is struggling, and
-- barely when the side is already flying. Only the result morale delta changes vs
-- 20260704_playmanager_match_player_events.sql (adds v_pressure + scales the
-- psychologist-derived win/draw/loss morale terms by it).
