-- PlayManager fix: restore the canonical league simulation (tactics + injuries +
-- readiness + ticket-price economy) as the last-wins definition.
--
-- Problem: pm_simulate_league_round is redefined across four migrations. The most
-- feature-complete version lives in 20260614_playmanager_finance_deepening.sql
-- (tactics, injuries, readiness AND ticket-price-driven attendance/income), but two
-- later migrations clobber it:
--   * matchday_mechanics.sql  -> tactics + injuries but NO ticket-price economy
--   * season_flow.sql         -> simple version, none of the above
-- Lexicographically season_flow wins, so the simple version is live and the entire
-- matchday/finance match system is inert (the readiness/tacticalStyle/injuryUpdate/
-- ticketPrice fields the TS layer reads are always undefined).
--
-- Worse, the finance_deepening and matchday versions each had an extra closing paren
-- in the fan_mood expression — a syntax error — so those richer functions never
-- created. This migration re-asserts the finance-based version (paren fixed) as the
-- final, last-wins definition so the intended mechanics are live.
--
-- Depends on: pm_match_settings, pm_finance_state, pm_season_state, pm_finalize_season,
-- and the injury_matches/morale columns (all from earlier 20260613/20260614 migrations).
...
begin
  ...
  set
    ...
    injury_matches = case
      when p.status = 'active' and coalesce(s.shirt_number, 99) <= 11
