-- PlayManager: give weak_foot (1-5 stars) a real match effect, the last
-- EA-style card attribute that was still display-only. Same pattern as
-- skill_moves (team-XI average multiplier, 3-star baseline) but narrower in
-- scope: weak foot is a finishing/shooting trait, not general flair, so it
-- only multiplies the ATTACK lane (skill_moves still multiplies
-- attack+wing+central). Magnitude is half of skill_moves (±0.8%/star vs
-- ±1.5%/star) to avoid double-stacking the same lane too hard.
-- Mirrored in the TS engine (match-engine.ts, championships + cups) in the
-- same commit.
