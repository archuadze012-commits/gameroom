-- PlayManager: Talent Classes — the raw 1–12 talent value now groups into 6 named
-- identity tiers, and each class carries a gameplay weight beyond the growth cap:
--
--   class          talent  age offset   decay starts (was 32 / 36)
--   ─────────────  ──────  ──────────   ────────────────────────
--   legend         12      +4           36 / 40
--   rising_star    11      +3           35 / 39
--   world_class    10      +2           34 / 38
--   elite          7–9     +1           33 / 37
--   star           4–6      0           32 / 36
--   pro            1–3     -1           31 / 35
--
-- The age offset shifts BOTH age-decay thresholds (talent loss + stat drop), so a
-- higher class peaks later and resists the decline longer (peak window + decay
-- resistance, one mechanism). The offset changes WHEN decay triggers, never the
-- per-position drop magnitude. TS mirror: src/lib/playmanager/talent.ts.

-- ── 1. Open the talent range to 12 (legend tier) ────────────────────────────
alter table public.pm_players
  drop constraint if exists pm_players_talent_check;

alter table public.pm_players
  add constraint pm_players_talent_check
  check (talent between 1 and 12);

-- ── 2. Class + age-offset helpers (mirror talent.ts) ────────────────────────
create or replace function public.pm_player_talent_class(
  p_talent smallint
) returns text
language sql
immutable
as $$
  select case
    when p_talent >= 12 then 'legend'
    when p_talent = 11 then 'rising_star'
    when p_talent = 10 then 'world_class'
    when p_talent between 7 and 9 then 'elite'
    when p_talent between 4 and 6 then 'star'
    else 'pro'
  end;
$$;

create or replace function public.pm_player_talent_class_age_offset(
  p_talent smallint
) returns integer
language sql
immutable
as $$
  select case
    when p_talent >= 12 then 4
    when p_talent = 11 then 3
    when p_talent = 10 then 2
    when p_talent between 7 and 9 then 1
    when p_talent between 4 and 6 then 0
    else -1
  end;
$$;

grant execute on function public.pm_player_talent_class(smallint) to authenticated, service_role;
grant execute on function public.pm_player_talent_class_age_offset(smallint) to authenticated, service_role;

-- ── 3. Growth cap: add the legend tier (talent 12 → +34) ────────────────────
...
-- ── 4. Aging engine: class-aware decay thresholds ───────────────────────────
-- Rebuilt from the live pm_advance_time (stat + talent decay), with the 32/36
-- thresholds shifted by each player's class age offset.
...
begin
  ...
  with aging as (
    select
      ...
      -- Class shifts the decay thresholds: t1 = 32 + offset, t2 = 36 + offset.
