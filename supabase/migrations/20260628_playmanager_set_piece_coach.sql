-- PlayManager: set-piece coach + set-piece scoring channel.
--
-- Adds a new staff role `set_piece_coach` and a dedicated set-piece xG channel to
-- the league match engine. Set-pieces are driven by PHY (aerial), SHO (dead-ball
-- finishing) and PAS (delivery) — already exposed by pm_team_match_profile.stats —
-- and the coach amplifies the attacking side (+4%/level, capped at +60%). Mirrors
-- the TS engine (src/lib/playmanager/match-engine.ts) used for cups.

-- ── 1. Allow the new role in hire cost + hire validation ────────────────────────
...
-- ── 2. League match engine: add the set-piece xG channel ────────────────────────
-- Reproduces pm_simulate_league_round verbatim with ONLY the set-piece additions:
--   • read set_piece_coach level into v_setpiece_pct
--   • compute home/away set-piece terms from PHY/SHO/PAS vs defence/keeper
--   • add the (clamped) term to home/away xG and expose it in matchEngine JSON
...
begin
  ...
  into v_doctor_recovery_pct, v_physio_recovery_pct, v_psychologist_morale_pct, v_setpiece_pct
  ...
  -- Set-piece channel: aerial (PHY) + dead-ball finishing (SHO) + delivery (PAS),
  -- amplified by the set-piece coach. The synthetic opponent gets a neutral
  -- set-piece profile derived from its lane strengths.
  ...
  v_opp_sp_attack := (v_opp_attack + v_opp_central + v_opp_defense) / 3;
