-- PlayManager match engine — tactical depth + squad/style compatibility.
--
-- Two layers, both inside the existing engine:
--
--  1) TACTICAL MATCHUP (rock-paper-scissors): pressing > possession > counter >
--     pressing. The opponent's style is derived deterministically from its form +
--     round, so it is scoutable. Reading it right is worth ~0.38 xG. Defensive
--     line / tempo / focus are independent risk-reward dials layered on top.
--
--  2) STYLE COMPATIBILITY: a style only pays off if the STARTING XI suits it.
--     pm_team_match_profile now also returns the XI's average PAC/SHO/PAS/DRI/
--     DEF/PHY (and the defenders' pace). The sim builds a `style_fit` factor
--     (0.5–1.5) from how specialised the squad is toward the chosen style and
--     MULTIPLIES the style bonus + matchup by it. So a pacey, physical squad
--     makes pressing devastating; a technical squad makes possession sing; the
--     wrong personnel blunts the same tactic. 'balanced' keys off overall stats,
--     so its fit is always ~1.0 — never boosted, never punished (its identity:
--     works with any squad, but no ceiling).
--     A concrete "other aspect": a HIGH defensive line concedes far less when
--     the defenders are quick, and much more when they are slow.
--
-- Squad quality (the OVR-driven lane terms) is still the baseline; tactics +
-- compatibility add the decisive strategic layer on top.

-- Ensure the TAC (tactical intelligence) column exists; the engine reads it.
alter table public.pm_players add column if not exists tac smallint;

-- ── Team match profile: now also exposes the XI's average raw stats ──────────
...
-- ── League simulation: tactical matchup + style compatibility ────────────────
