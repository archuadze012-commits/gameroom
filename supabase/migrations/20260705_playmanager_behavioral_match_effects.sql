-- PlayManager: give the 5 previously display-only behavioural attributes a real
-- match effect, mirroring how `consistency` already damps rating variance and
-- how `traits` already grant lane bonuses. Each attribute maps to ONE lane in
-- pm_team_match_profile (the SQL engine behind pm_simulate_league_round), a
-- team-XI average centered at 60 (neutral), clamped so it stays comparable to
-- the existing trait bonuses:
--   composure   → attack   (calm finishing)          (avg-60)*0.10, ±5
--   vision      → central  (final-third creativity)  (avg-60)*0.10, ±5
--   positioning → defense  (marking / shape)         (avg-60)*0.10, ±5
--   aggression  → midfield + (pressing / recovery)   (avg-60)*0.10, ±5
--               → defense  - (leaves gaps behind)    (avg-60)*0.03, ±2
--   stamina     → readiness (holds level late)       (avg-60)*0.08, ±4
-- `consistency` is unchanged (it lives in pm_match_player_events). The
-- behavioural jsonb is threaded through the auto-sub CTE chain exactly like
-- `stats`, so bonuses reflect the actually-fielded XI (subs included).
-- The TS mirror (match-engine.ts, used by championships + cups) is kept in sync.
...
as $function$
  ...
  select jsonb_build_object(
    'attack', round(attack * (select mult from skill), 2),
...
revoke all on function public.pm_team_match_profile(uuid) from public, anon, authenticated;
