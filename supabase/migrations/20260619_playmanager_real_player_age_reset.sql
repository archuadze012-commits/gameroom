-- PlayManager: reset all real-player ages and start aging only after purchase.

alter table public.pm_players
  add column if not exists age_started_total_days integer
    check (age_started_total_days is null or age_started_total_days > 0);

update public.pm_players p
set
  age = 18,
  age_started_total_days = case
    when p.is_real and p.owner_id is not null then coalesce(c.total_days, 1)
    else null
  end
from public.pm_calendar c
where p.is_real
  and p.owner_id = c.team_id;

update public.pm_players
set
  age = 18,
  age_started_total_days = null
where is_real
