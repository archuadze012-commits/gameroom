-- PlayManager: restore three staff-role match effects that a refactor silently
-- dropped. 20260629a wired finance_manager (income), set_piece_coach (set-piece
-- xG) and head_coach (match readiness) into pm_simulate_league_round, but the
-- 20260630_position_fit rebuild reconstructed the function from an older base and
-- left all three out. Every redefinition since (stadium, match_player_events,
-- psychologist_niche) carried that gap forward, so as of 20260707 the three roles
-- are cosmetic: managers pay their wages but get no effect.
--
-- This rebuilds the CURRENT (20260707) function verbatim and grafts back ONLY the
-- three staff effects — no other engine behaviour changes.
--
-- ⚠️ MAINTAINERS: if you ever CREATE OR REPLACE pm_simulate_league_round again,
-- you MUST carry forward the six-role staff select, v_head_coach_readiness in
-- v_match_readiness, the v_*_setpiece terms in the xG, and v_finance_income_pct
-- in v_income. Grep for "STAFF-EFFECT" below to find every touch point.
...
declare
  ...
  -- STAFF-EFFECT: restored role bonuses (set-piece coach, head coach, finance manager).
  v_setpiece_pct integer := 0;
  v_head_coach_readiness integer := 0;
  v_finance_income_pct integer := 0;
  ...
  -- STAFF-EFFECT: set-piece intermediate values.
  v_sp_attack numeric := 0;
  v_sp_defense numeric := 0;
  v_opp_sp_attack numeric := 0;
  v_opp_sp_defense numeric := 0;
  v_home_setpiece numeric := 0;
  v_away_setpiece numeric := 0;
...
begin
  ...
  -- STAFF-EFFECT: six-role staff select. doctor/physio/psychologist were the only
  -- three that survived the refactor; set_piece_coach, head_coach and
  -- finance_manager are restored here.
  select
    ...
    coalesce(sum(case when role_key = 'set_piece_coach' then level * 4 else 0 end), 0)::integer,
    coalesce(sum(case when role_key = 'head_coach' then level * 2 else 0 end), 0)::integer,
    coalesce(sum(case when role_key = 'finance_manager' then level * 3 else 0 end), 0)::integer
  into v_doctor_recovery_pct, v_physio_recovery_pct, v_psychologist_morale_pct, v_setpiece_pct, v_head_coach_readiness, v_finance_income_pct
  ...
  -- STAFF-EFFECT: head_coach readiness bonus (+level*2) restored on match readiness.
  v_match_readiness := greatest(35, least(100, round(((v_avg_ovr - 55) * 1.2) + (v_avg_morale * 0.42) - (v_avg_fatigue * 0.38) - (v_injured_count * 7) + v_head_coach_readiness)::numeric));
  ...
  -- STAFF-EFFECT: set_piece_coach lifts the home set-piece threat (level*4%, capped
  -- at +60%); both sides get a set-piece xG term that flows into the totals below.
  v_sp_attack := (v_phy * 0.45 + v_sho * 0.35 + v_pas * 0.20) * (1 + least(60, v_setpiece_pct)::numeric / 100.0);
  v_sp_defense := v_defense * 0.5 + v_phy * 0.3 + v_keeper * 0.2;
  v_opp_sp_attack := v_opp_base;
  v_opp_sp_defense := v_opp_defense * 0.6 + v_opp_keeper * 0.4;
  v_home_setpiece := greatest(-0.45, least(0.6, (v_sp_attack - v_opp_sp_defense) / 55));
  v_away_setpiece := greatest(-0.45, least(0.6, (v_opp_sp_attack - v_sp_defense) / 55));
  ...
  -- Psychologist niche: pressure factor from form. Low form ⇒ psychologist matters
  -- more (softens losses, lifts draws); high form ⇒ minimal effect.
  v_pressure := case
    when coalesce(v_team_form, 70) < 50 then 1.6
    when coalesce(v_team_form, 70) < 60 then 1.3
    when coalesce(v_team_form, 70) < 72 then 1.0
    when coalesce(v_team_form, 70) < 85 then 0.7
    else 0.4
  end;
  v_morale_delta := case
    when v_result = 'W' then 6 + round(v_morale_win_bonus * least(1.0, v_pressure))::integer
    when v_result = 'D' then 1 + round(v_morale_draw_bonus * v_pressure)::integer
    else -(greatest(1, 5 - round(v_morale_loss_reduction * v_pressure)::integer))
  end;
  ...
  -- STAFF-EFFECT: finance_manager boosts real credited matchday income (+level*3%),
  -- matching the projected figure city-data.ts already shows.
  v_income := (round(((v_attendance * coalesce(v_finance.ticket_price, 28)) + (case when v_result = 'W' then 70000 when v_result = 'D' then 30000 else 0 end))::numeric * (1 + v_finance_income_pct::numeric / 100.0) / 500) * 500)::bigint;
  ...
  return jsonb_build_object(
    ...
    'matchEngine', jsonb_build_object(
      ...
      -- STAFF-EFFECT: expose the restored set-piece contribution for the UI.
      'setPiece', jsonb_build_object(
        'home', round(v_home_setpiece, 2),
        'away', round(v_away_setpiece, 2),
        'coachPct', v_setpiece_pct
      ),
