-- head_coach ("მენეჯერის ასისტენტი") readiness now feeds the league match
-- engine: declare v_head_coach_readiness, read sum(level*2), add to v_match_readiness.
-- Mirrors the TS cup engine (buildMatchProfile readinessFlat). OVR-growth effect
-- (pm_train_player) is unchanged.
...
begin
  ...
  into v_doctor_recovery_pct, v_physio_recovery_pct, v_psychologist_morale_pct, v_setpiece_pct, v_head_coach_readiness
  ...
  v_opp_sp_attack := v_opp_base;  -- neutral set-piece threat pegged to overall quality (no open-play double-count, no defence-boosts-attack)
