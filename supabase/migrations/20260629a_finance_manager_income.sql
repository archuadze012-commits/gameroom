-- Fix: finance_manager's projectedIncomePct (level*3%) was display-only — the UI
-- showed boosted projected income (city-data.ts boostedProjectedMatchdayIncome)
-- but pm_simulate_league_round credited matchday income WITHOUT it. Apply the
-- bonus to the real credited income so the staff role does what it advertises.
CREATE OR REPLACE FUNCTION public.pm_simulate_league_round(p_team_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  -- Matchday income, boosted by the finance_manager (level*3%, matches the UI's
  -- projected income promise in city-data.ts).
  ...
  -- Phase 3: the starting XI earns development XP for playing this match.
  perform public.pm_grant_match_development(p_team_id);
