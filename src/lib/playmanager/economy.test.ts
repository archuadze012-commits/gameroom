...
test('projected attendance stays inside stadium capacity', () => {
  assert.equal(
    getProjectedAttendance({ formPercent: 100, readiness: 100, ticketPrice: 10 }),
    STADIUM_CAPACITY,
  );
  ...
  assert.ok(
    expensiveTicketAttendance >= MIN_MATCHDAY_ATTENDANCE &&
      expensiveTicketAttendance < STADIUM_CAPACITY,
  );
});

test('projected matchday income clamps impossible attendance and ticket prices', () => {
  assert.equal(
    getProjectedMatchdayIncome({ attendance: 100_000, ticketPrice: 999 }),
    STADIUM_CAPACITY * MAX_TICKET_PRICE_GEL,
  );
});
