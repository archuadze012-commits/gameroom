...
// Matches the background image / desktop host aspect ratio (1672 / 941).
const DESIGN_ASPECT = 1672 / 941;
...
export function PlayManagerCityCanvas({ className, buildings, backgroundUrl, hud, onBuildingSelect }: PlayManagerCityCanvasProps) {
  ...
  useEffect(() => {
    ...
    async function mountThree() {
      ...
      const resize = () => {
        ...
        // "Contain" framing to match the CSS background-size:contain backdrop.
        // The whole design region (DESIGN_ASPECT × CAMERA_VIEW_HEIGHT) is always
        // fully visible and centered; the leftover axis is padded by the frame's
        // dark backdrop — so no edge building is ever cropped at a non-16:9 frame.
        const designWidth = CAMERA_VIEW_HEIGHT * DESIGN_ASPECT;
        let viewWidth: number;
        let viewHeight: number;
        if (aspect > DESIGN_ASPECT) {
          // Host wider than design → fit full height, pad the extra width.
          viewHeight = CAMERA_VIEW_HEIGHT;
          viewWidth = CAMERA_VIEW_HEIGHT * aspect;
        } else {
          // Host taller than design → fit full width, pad the extra height.
          viewWidth = designWidth;
          viewHeight = designWidth / aspect;
        }
      ...
      // Select directly from the tap/click position. Touch devices fire no
      // hover events before the tap, so we can't rely on the rAF-computed
      // `hovered` — we update the pointer from the event and raycast right here.
      const selectAt = (event: PointerEvent) => {
        ...
        const spriteKey = hit?.userData.spriteKey;
      ...
      cleanup = () => {
        ...
        renderer.domElement.removeEventListener('pointerdown', selectAt);
  ...
  return (
    <div
      className={`pm-three-host ${className || ''}`.trim()}
      ref={hostRef}
    >
      <div
...
async function createBuildingSprite(
  ...
  const texture = building.spriteUrl
    ? await loader.loadAsync(building.spriteUrl).catch(() => createFallbackTexture(THREE, building.tone))
