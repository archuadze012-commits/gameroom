...
import { mockGames, type MockUser } from "@/lib/mock-data";
import { RoleBadge, type UserRole } from "@/components/role-badge";


type Props = { user: MockUser };
...
export function PlayerCard({ user }: Props) {
  ...
  const games = user.games.slice(0, 3).map((g) => mockGames.find((mg) => mg.slug === g.slug)).filter(Boolean) as typeof mockGames;
    ...
    <Link href={`/profile/${user.username}`} className="group">
      <Card className="relative h-52 overflow-hidden border-border/60 transition-all hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5">
        ...
        <div className="absolute bottom-0 left-0 right-0 flex items-end justify-between p-3">
          <div>
            <RoleBadge username={user.username} defaultRole={user.role as UserRole | undefined} />
            <p className="font-bold text-white leading-tight">{user.displayName}</p>
            <p className="text-xs text-white/60">{user.region}{user.voiceChat ? " · 🎙" : ""}</p>
          </div>
          <div className="flex items-center gap-1">
            {games.map((g) => (
              <div key={g.slug} className="flex h-8 w-8 items-center justify-center rounded-lg bg-black/50 backdrop-blur-sm">
