...
export function OnboardingChecklist({ hasProfile, hasGames, hasFollows, hasPush, hasClaimedDaily, isAdmin }: Props) {
  ...
  const steps: Step[] = [
    ...
    { key: "games", done: srv(hasGames), icon: Gamepad2, label: "აირჩიე შენი თამაშები", spot: [{ sel: "nav-games", label: "თამაშები — აქ" }, { sel: "more-mobile", label: "გახსენი «მეტი» → თამაშები" }], href: "/games", cta: "არჩევა" },
    { key: "follow", done: srv(hasFollows), icon: UserPlus, label: "გაჰყევი 3 მოთამაშეს", spot: [{ sel: "nav-search", label: "ძებნა — იპოვე მოთამაშეები" }, { sel: "more-mobile", label: "გახსენი «მეტი» → ძებნა" }], href: "/search", cta: "პოვნა" },
    { key: "lfg", done: visited.lfg, icon: Users, label: "იპოვე გუნდი — LFG", spot: [{ sel: "nav-lfg", label: "ლოკალი (LFG) — აქ" }, { sel: "more-mobile", label: "გახსენი «მეტი» → ლოკალი" }], href: "/lfg", cta: "ნახვა", onClick: () => markVisited("lfg") },
