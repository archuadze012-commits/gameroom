...
export type RunCityActionResult =
  ...
  | {
      success: false;
      error: 'unauthenticated' | 'team_missing' | 'invalid_facility' | 'insufficient_funds' | 'facility_locked' | 'unavailable';
    };
...
export async function runPlayManagerCityAction(input: {
  ...
  if (calendar) detailParts.push(`W${calendar.weekNo} D${calendar.dayNo}`);
...
export async function buyPlayManagerMarketPlayer(playerKey: string): Promise<PlayManagerPlayerActionResult> {
  ...
  await logPlayManagerEvent({
    ...
    detail: `${price.toLocaleString('ka-GE')} ₾ deal${refund > 0 ? ` · refund ${refund.toLocaleString('ka-GE')} ₾` : ''}${calendar ? ` · W${calendar.weekNo} D${calendar.dayNo}` : ''}`,
  });
  revalidatePath('/playmanager');
  return {
    success: true,
...
export async function trainPlayManagerPlayer(playerId: string): Promise<PlayManagerPlayerActionResult> {
  ...
  const db = asPlayManagerDb(createSupabaseAdminClient());
  const actionContext = await getActionContext(user.id, team.id);
  const { error } = await db.rpc('pm_train_player', {
    p_team_id: team.id,
  ...
  await logPlayManagerEvent({
    ...
    detail: `OVR +1 · XP +${xpReward}${calendar ? ` · W${calendar.weekNo} D${calendar.dayNo}` : ''}`,
  });
  revalidatePath('/playmanager');
  return { success: true, message: `OVR გაიზარდა +1-ით · XP +${xpReward}` };
}
...
export async function signPlayManagerAcademyProspect(prospectId: string): Promise<PlayManagerPlayerActionResult> {
  ...
  await logPlayManagerEvent({
    ...
    detail: `${data?.cost?.toLocaleString('ka-GE') ?? 0} ₾ signing${refund > 0 ? ` · refund ${refund.toLocaleString('ka-GE')} ₾` : ''}${calendar ? ` · W${calendar.weekNo} D${calendar.dayNo}` : ''}`,
  });
  revalidatePath('/playmanager');
  return {
    success: true,
...
export async function sellPlayManagerPlayer(playerId: string): Promise<PlayManagerPlayerActionResult> {
  ...
  await logPlayManagerEvent({
    ...
    detail: `${(data?.amount ?? 0).toLocaleString('ka-GE')} ₾ დაბრუნდა ბიუჯეტში${calendar ? ` · W${calendar.weekNo} D${calendar.dayNo}` : ''}`,
  });
  revalidatePath('/playmanager');
...
export async function negotiatePlayManagerSponsor(): Promise<PlayManagerPlayerActionResult> {
  ...
  await logPlayManagerEvent({
    ...
    detail: `${(data?.sponsorWeeklyAmount ?? 0).toLocaleString('ka-GE')} ₾ weekly${calendar ? ` · W${calendar.weekNo} D${calendar.dayNo}` : ''}`,
  });
  revalidatePath('/playmanager');
  return {
    success: true,
