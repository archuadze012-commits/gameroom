...
export const metadata = { title: "პრაქტიკული თამაშები | კლანი", robots: { index: false } };

export default async function ClanScrimsPage({
  params,
}: {
  ...
  const items = await getClanTournaments(ctx.supabase, ctx.game.id, ctx.clan.id, true);
    ...
    <div className="relative min-h-[calc(100vh-4rem)] bg-transparent">
      ...
      <div className="container relative mx-auto max-w-4xl px-4 py-8 lg:py-10">
        ...
        <p className="mb-4 text-[12.5px] text-white/50">
          ვარჯიშის მატჩები — ტურნირის სისტემით, პრიზის გარეშე ან მიზერული NC-ით.
        </p>
        <ClanTournamentList clanSlug={clanSlug} canRegister={ctx.canManage} items={items} kind="scrims" />
      </div>
    </div>
  );
}
