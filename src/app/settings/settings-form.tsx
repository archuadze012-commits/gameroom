...
export function SettingsForm({ games = [] }: { games?: Game[] }) {
  ...
  const nextDisplayNameChangeAt = displayNameChangedAt
    ...
    : null;
  const displayNameLocked = !!nextDisplayNameChangeAt && nextDisplayNameChangeAt.getTime() > new Date().getTime();
  const displayNameDaysLeft = nextDisplayNameChangeAt
    ? Math.max(1, Math.ceil((nextDisplayNameChangeAt.getTime() - new Date().getTime()) / (24 * 60 * 60 * 1000)))
    : 0;
