...
export function SettingsForm({ games = [] }: { games?: Game[] }) {
  ...
  const currentNow = now ?? new Date().getTime();
  const displayNameLocked = !!nextDisplayNameChangeAt && nextDisplayNameChangeAt.getTime() > currentNow;
  const displayNameDaysLeft = nextDisplayNameChangeAt
    ? Math.max(1, Math.ceil((nextDisplayNameChangeAt.getTime() - currentNow) / (24 * 60 * 60 * 1000)))
    : 0;
