...
export function SettingsForm({ games = [] }: { games?: Game[] }) {
  ...
  const displayNameLocked = !!nextDisplayNameChangeAt && nextDisplayNameChangeAt.getTime() > (now || new Date().getTime());
  const displayNameDaysLeft = nextDisplayNameChangeAt
    ? Math.max(1, Math.ceil((nextDisplayNameChangeAt.getTime() - (now || new Date().getTime())) / (24 * 60 * 60 * 1000)))
    : 0;
